...
```javascript
store.info.basic // player/teitoku basic information, name, id, level, exp, etc.
ships
...
```javascript
store.battle.result.rank // String
boss // Boolean
map // Integer(2-3 => 23)
...
```javascript
store.sortie.combinedFlag // Integer, api_combined_flag
...
```javascript
store.misc.canNotify // will be true the first time log in to HQ screen
```
...
```javascript
action.type // `@@Request/kcsapi/<api>`
method // 'GET' | 'POST' | ...
path // `/kcsapi/<api>`
body // Request body
```
...
```javascript
action.type // `@@Response/kcsapi/<api>`
method // 'GET' | 'POST' | ...
path // `/kcsapi/<api>`
body // Response body
...
* `actionNameBase`: _String_
* `promiseGenerator`: _function_ with no argument, returning a promise
